# HaiBot Installer for Windows (self-contained: includes uv download via GitHub)
# Usage: irm <url>/install.ps1 | iex
#    or: .\install.ps1 [-Version X.Y.Z] [-FromSource] [-SourceDir DIR]
#                            [-Extras "llamacpp,mlx"] [-UvPath PATH]
#
# Installs HaiBot into ~/.haibot with a uv-managed Python environment.
...
# ── Defaults ──────────────────────────────────────────────────────────────────
$CopawHome     = if ($env:HAIBOT_HOME) { $env:HAIBOT_HOME } else { Join-Path $HOME ".haibot" }
$CopawVenv     = Join-Path $CopawHome "venv"
$CopawBin      = Join-Path $CopawHome "bin"
$PythonVersion = "3.12"
$CopawRepo     = "https://github.com/agentscope-ai/HaiBot.git"

# ── Colors ────────────────────────────────────────────────────────────────────
function Write-Info { param([string]$Message) Write-Host "[haibot] " -ForegroundColor Green  -NoNewline; Write-Host $Message }
function Write-Warn { param([string]$Message) Write-Host "[haibot] " -ForegroundColor Yellow -NoNewline; Write-Host $Message }
function Write-Err  { param([string]$Message) Write-Host "[haibot] " -ForegroundColor Red    -NoNewline; Write-Host $Message }
function Stop-WithError { param([string]$Message) Write-Err $Message; exit 1 }

# ── Help ──────────────────────────────────────────────────────────────────────
if ($Help) {
    @"
HaiBot Installer for Windows
...
Environment:
  HAIBOT_HOME            Installation directory (default: ~/.haibot)
"@
    exit 0
}

Write-Host "[haibot] " -ForegroundColor Green -NoNewline
Write-Host "Installing HaiBot into " -NoNewline
...
function Prepare-Console {
    ...
    $consoleDest = Join-Path $RepoDir "src\haibot\console"
...
$VenvCopaw = Join-Path $CopawVenv "Scripts\haibot.exe"

if ($FromSource) {
    if ($SourceDir) {
        $SourceDir = (Resolve-Path $SourceDir).Path
        Write-Info "Installing HaiBot from local source: $SourceDir"
        ...
        if (-not (Get-Command git -ErrorAction SilentlyContinue)) {
            Stop-WithError "git is required for -FromSource without a local directory. Please install Git from https://git-scm.com/ or pass a local path: .\install.ps1 -FromSource -SourceDir C:\path\to\HaiBot"
        }
        Write-Info "Installing HaiBot from source (GitHub)..."
        $cloneDir = Join-Path $env:TEMP "haibot-install-$(Get-Random)"
        try {
    ...
    $package = "haibot"
    if ($Version) { $package = "haibot==$Version" }

    Write-Info "Installing ${package}${ExtrasSuffix} from PyPI..."
    uv pip install "${package}${ExtrasSuffix}" --python $VenvPython --prerelease=allow --quiet --refresh-package haibot
    if ($LASTEXITCODE -ne 0) { Stop-WithError "Installation failed" }
}

# Verify the CLI entry point exists
if (-not (Test-Path $VenvCopaw)) { Stop-WithError "Installation failed: haibot CLI not found in venv" }

Write-Info "HaiBot installed successfully"

# Check console availability (for PyPI installs, check the installed package)
if (-not $script:ConsoleAvailable) {
    $consoleCheck = & $VenvPython -c "import importlib.resources, haibot; p=importlib.resources.files('haibot')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>&1
...
$wrapperPath = Join-Path $CopawBin "haibot.ps1"
$wrapperContent = @'
# HaiBot CLI wrapper — delegates to the uv-managed environment.
$ErrorActionPreference = "Stop"

$CopawHome = if ($env:HAIBOT_HOME) { $env:HAIBOT_HOME } else { Join-Path $HOME ".haibot" }
$RealBin   = Join-Path $CopawHome "venv\Scripts\haibot.exe"

if (-not (Test-Path $RealBin)) {
    Write-Error "HaiBot environment not found at $CopawHome\venv"
...
$cmdWrapperPath = Join-Path $CopawBin "haibot.cmd"
$cmdWrapperContent = @"
@echo off
REM HaiBot CLI wrapper — delegates to the uv-managed environment.
set "HAIBOT_HOME=%HAIBOT_HOME%"
if "%HAIBOT_HOME%"=="" set "HAIBOT_HOME=%USERPROFILE%\.haibot"
set "REAL_BIN=%HAIBOT_HOME%\venv\Scripts\haibot.exe"
if not exist "%REAL_BIN%" (
    echo Error: HaiBot environment not found at %HAIBOT_HOME%\venv >&2
...
if (-not $isAlreadyAdded) {
    ...
    try {
        ...
        Write-Host ""
        Write-Host "ACTION REQUIRED: You must manually add the path to use HaiBot."
...
# ── Done ──────────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "HaiBot installed successfully!" -ForegroundColor Green
Write-Host ""
...
Write-Host ""
Write-Host "  haibot init" -ForegroundColor White -NoNewline; Write-Host "       # first-time setup"
Write-Host "  haibot app"  -ForegroundColor White -NoNewline; Write-Host "        # start HaiBot"
Write-Host ""
Write-Host "To upgrade later, re-run this installer."
Write-Host "To uninstall, run: " -NoNewline
Write-Host "haibot uninstall" -ForegroundColor White
