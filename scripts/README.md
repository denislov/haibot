...
- Builds the console frontend (`console/`), copies `console/dist` to `src/haibot/console/dist`, then builds the wheel. Output: `dist/*.whl`.
...
- Default tag: `haibot:latest`. Uses `deploy/Dockerfile` (multi-stage: builds console then Python app).
- Example: `bash scripts/docker_build.sh myreg/haibot:v1 --no-cache`.
