...
info()  { printf "${GREEN}[haibot]${RESET} %s\n" "$*"; }
warn()  { printf "${YELLOW}[haibot]${RESET} %s\n" "$*"; }
error() { printf "${RED}[haibot]${RESET} %s\n" "$*" >&2; }
die()   { error "$@"; exit 1; }

# ── Defaults ──────────────────────────────────────────────────────────────────
HAIBOT_HOME="${HAIBOT_HOME:-$HOME/.haibot}"
HAIBOT_VENV="$HAIBOT_HOME/venv"
HAIBOT_BIN="$HAIBOT_HOME/bin"
PYTHON_VERSION="3.12"
HAIBOT_REPO="https://github.com/agentscope-ai/HaiBot.git"
...
printf "${GREEN}[haibot]${RESET} Installing HaiBot into ${BOLD}%s${RESET}\n" "$HAIBOT_HOME"
...
if [ -d "$HAIBOT_VENV" ]; then
    info "Existing environment found, upgrading..."
else
    info "Creating Python $PYTHON_VERSION environment..."
fi

uv venv "$HAIBOT_VENV" --python "$PYTHON_VERSION" --quiet

# Verify the venv was created
[ -x "$HAIBOT_VENV/bin/python" ] || die "Failed to create virtual environment"
info "Python environment ready ($("$HAIBOT_VENV/bin/python" --version))"
...
## Ensure console frontend assets are in src/haibot/console/ for source installs.
...
prepare_console() {
    ...
    local console_dest="$repo_dir/src/haibot/console"
...
cleanup_console() {
    ...
    if [ "$_CONSOLE_COPIED" = 1 ]; then
        rm -rf "$repo_dir/src/haibot/console/"*
    fi
}

if [ "$FROM_SOURCE" = true ]; then
    if [ -n "$SOURCE_DIR" ]; then
        info "Installing HaiBot from local source: $SOURCE_DIR"
        prepare_console "$SOURCE_DIR"
        info "Installing package from source..."
        uv pip install "${SOURCE_DIR}${EXTRAS_SUFFIX}" --python "$HAIBOT_VENV/bin/python" --prerelease=allow --index-url "$PYPI_MIRROR"
        cleanup_console "$SOURCE_DIR"
    else
        info "Installing HaiBot from source (GitHub)..."
        CLONE_DIR="$(mktemp -d)"
        trap 'rm -rf "$CLONE_DIR"' EXIT
        git clone --depth 1 "$HAIBOT_REPO" "$CLONE_DIR"
        prepare_console "$CLONE_DIR"
        info "Installing package from source..."
        uv pip install "${CLONE_DIR}${EXTRAS_SUFFIX}" --python "$HAIBOT_VENV/bin/python" --prerelease=allow --index-url "$PYPI_MIRROR"
        # CLONE_DIR is cleaned up by trap; no need for cleanup_console
    fi
else
    PACKAGE="haibot"
    if [ -n "$VERSION" ]; then
        PACKAGE="haibot==$VERSION"
    fi

    info "Installing ${PACKAGE}${EXTRAS_SUFFIX} from PyPI..."
    uv pip install "${PACKAGE}${EXTRAS_SUFFIX}" --python "$HAIBOT_VENV/bin/python" --prerelease=allow --quiet --index-url "$PYPI_MIRROR" --refresh-package haibot
fi

# Verify the CLI entry point exists
[ -x "$HAIBOT_VENV/bin/haibot" ] || die "Installation failed: haibot CLI not found in venv"
info "HaiBot installed successfully"
...
if [ "$_CONSOLE_AVAILABLE" = 0 ]; then
    ...
    CONSOLE_CHECK="$("$HAIBOT_VENV/bin/python" -c "import importlib.resources, haibot; p=importlib.resources.files('haibot')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>/dev/null || echo 'no')"
...
cat > "$HAIBOT_BIN/haibot" << 'WRAPPER'
#!/usr/bin/env bash
# HaiBot CLI wrapper — delegates to the uv-managed environment.
set -euo pipefail

HAIBOT_HOME="${HAIBOT_HOME:-$HOME/.haibot}"
REAL_BIN="$HAIBOT_HOME/venv/bin/haibot"
...
chmod +x "$HAIBOT_BIN/haibot"
info "Wrapper created at $HAIBOT_BIN/haibot"

# ── Step 5: Update PATH in shell profile ─────────────────────────────────────
PATH_ENTRY="export PATH=\"\$HOME/.haibot/bin:\$PATH\""

add_to_profile() {
    local profile="$1"
    if [ -f "$profile" ] && grep -qF '.haibot/bin' "$profile"; then
...
# ── Done ──────────────────────────────────────────────────────────────────────
echo ""
printf "${GREEN}${BOLD}HaiBot installed successfully!${RESET}\n"
echo ""

# Install summary
printf "  Install location:  ${BOLD}%s${RESET}\n" "$HAIBOT_HOME"
printf "  Python:            ${BOLD}%s${RESET}\n" "$("$HAIBOT_VENV/bin/python" --version 2>&1)"
...
echo "Then run:"
echo ""
printf "  ${BOLD}haibot init${RESET}       # first-time setup\n"
printf "  ${BOLD}haibot app${RESET}        # start HaiBot\n"
echo ""
printf "To upgrade later, re-run this installer.\n"
printf "To uninstall, run: ${BOLD}haibot uninstall${RESET}\n"
