...
"""
Create a temporary conda env, install HaiBot from a wheel, run conda-pack.
...
def _pick_wheel(wheel_arg: str | None) -> Path:
    ...
    wheels = sorted(
        (REPO_ROOT / "dist").glob("haibot-*.whl"),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Conda-pack HaiBot (temp env).",
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "installing haibot[full]"
            )
            needs_llama_compile = True

        # Install haibot with all dependencies
