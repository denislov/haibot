...
$Archive = Join-Path $Dist "haibot-env.zip"
$Unpacked = Join-Path $Dist "win-unpacked"
$NsiPath = Join-Path $PackDir "haibot_desktop.nsi"
...
$VersionFile = Join-Path $RepoRoot "src\haibot\__version__.py"
$CurrentVersion = ""
...
$RunWheelBuild = $true
if ($CurrentVersion) {
  $wheelGlob = Join-Path $Dist "haibot-$CurrentVersion-*.whl"
  ...
  if ($existingWheels.Count -gt 0) {
    ...
    $oldWheels = Get-ChildItem -Path (Join-Path $Dist "haibot-*.whl") -ErrorAction SilentlyContinue
...
$LauncherBat = Join-Path $EnvRoot "HaiBot Desktop.bat"
@"
@echo off
cd /d "%~dp0"
...
REM Log level: env var HAIBOT_LOG_LEVEL or default to "info"
if not defined HAIBOT_LOG_LEVEL set "HAIBOT_LOG_LEVEL=info"

REM Set SSL certificate paths for packaged environment
REM Use temp file to avoid for /f blocking issue in bat scripts
set "CERT_TMP=%TEMP%\haibot_cert_%RANDOM%.txt"
...
if not exist "%USERPROFILE%\.haibot\config.json" (
  "%~dp0python.exe" -u -m haibot init --defaults --accept-security
)
"%~dp0python.exe" -u -m haibot desktop --log-level %HAIBOT_LOG_LEVEL%
"@ | Set-Content -Path $LauncherBat -Encoding ASCII

# Debug launcher .bat (shows console)
$DebugBat = Join-Path $EnvRoot "HaiBot Desktop (Debug).bat"
@"
@echo off
cd /d "%~dp0"
...
REM Debug mode: use debug log level by default (can override with HAIBOT_LOG_LEVEL)
if not defined HAIBOT_LOG_LEVEL set "HAIBOT_LOG_LEVEL=debug"
...
echo ====================================
echo HaiBot Desktop - Debug Mode
echo ====================================
echo Working Directory: %cd%
echo Python: "%~dp0python.exe"
echo PATH: %PATH%
echo Log Level: %HAIBOT_LOG_LEVEL%
...
echo [Launch] Starting HaiBot Desktop with log-level=%HAIBOT_LOG_LEVEL%...
echo Press Ctrl+C to stop
echo.
"%~dp0python.exe" -u -m haibot desktop --log-level %HAIBOT_LOG_LEVEL%
echo.
echo [Exit] HaiBot Desktop closed
pause
"@ | Set-Content -Path $DebugBat -Encoding ASCII

# VBScript launcher (no console window)
$LauncherVbs = Join-Path $EnvRoot "HaiBot Desktop.vbs"
@"
Set WshShell = CreateObject("WScript.Shell")
batPath = CreateObject("Scripting.FileSystemObject").GetParentFolderName(WScript.ScriptFullName) & "\HaiBot Desktop.bat"
...
if (-not $Version) {
  ...
  try {
    $Version = (& (Join-Path $EnvRoot "python.exe") -c "from importlib.metadata import version; print(version('haibot'))" 2>&1) -replace '\s+$', ''
...
Write-Host "[build_win] HAIBOT_VERSION=$Version OUTPUT_EXE will be under $Dist"
$OutInstaller = Join-Path (Join-Path $RepoRoot $Dist) "HaiBot-Setup-$Version.exe"
