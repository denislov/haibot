# HaiBot Desktop packaging scripts
...
# Output: dist/HaiBot-Setup-<version>.exe
# Creates two launchers:
#   - HaiBot Desktop.vbs (silent, no console window)
#   - HaiBot Desktop (Debug).bat (shows console for troubleshooting)
...
APP_ENV="$(pwd)/dist/HaiBot.app/Contents/Resources/env"
PYTHONPATH= PYTHONHOME="$APP_ENV" "$APP_ENV/bin/python" -m haibot desktop
```
...
When you **double-click** the .app and nothing appears, the launcher writes stderr/stdout to `~/.haibot/desktop.log`. Inspect that file for errors.
...
When users download the HaiBot macOS app (e.g. from Releases) as a `.app` (in a zip), macOS may show: *"Apple cannot verify that 'HaiBot' contains no malicious software"*. The app is not notarized. They can still open it as follows:

- **Right-click to open (recommended)**
  Right-click (or Control+click) the HaiBot app → **Open** → in the dialog click **Open** again. Gatekeeper will allow it; after that double-click works as usual.

- **Allow in System Settings**
  If still blocked, go to **System Settings → Privacy & Security**, find the message like *"HaiBot was blocked because it is from an unidentified developer"*, and click **Open Anyway** or **Allow**.

- **Remove quarantine attribute (not recommended for most users)**
  In Terminal: `xattr -cr /Applications/HaiBot.app` (or the path to the `.app` after unzipping). This clears the download quarantine flag; less safe than right-click → Open.
...
| File | Description |
|------|-------------|
| `build_common.py` | Create temporary conda env, install `haibot[full]` from a wheel, conda-pack; produces archive. |
| `build_macos.sh` | One-click: build wheel → build_common → unpack into HaiBot.app; optional zip. |
| `build_win.ps1` | One-click: build wheel → build_common → unpack → create VBS/BAT launchers → makensis installer. |
| `haibot_desktop.nsi` | NSIS script: pack `dist/win-unpacked`, add icons, and create shortcuts. |
