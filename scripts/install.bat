...
REM HaiBot Installer for Windows (cmd.exe / batch)
REM Usage: install.bat [-Version X.Y.Z] [-FromSource] [-SourceDir DIR]
REM                         [-Extras "llamacpp,mlx"] [-UvPath PATH] [-Help]
REM
REM Installs HaiBot into %USERPROFILE%\.haibot with a uv-managed Python environment.
...
REM ── Defaults ──────────────────────────────────────────────────────────────────
if defined HAIBOT_HOME (
    set "HAIBOT_HOME=%HAIBOT_HOME%"
) else (
    set "HAIBOT_HOME=%USERPROFILE%\.haibot"
)
set "HAIBOT_VENV=%HAIBOT_HOME%\venv"
set "HAIBOT_BIN=%HAIBOT_HOME%\bin"
set "PYTHON_VERSION=3.12"
set "HAIBOT_REPO=https://github.com/agentscope-ai/HaiBot.git"
...
REM ──── Help ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
:show_help
echo HaiBot Installer for Windows
echo.
...
echo.
echo Environment:
echo   HAIBOT_HOME            Installation directory (default: %%USERPROFILE%%\.haibot)
exit /b 0
...
echo [haibot] Downloading uv ^(!_DL_ARCH!^) from GitHub Releases...
...
if not errorlevel 1 (
    ...
    echo [haibot] curl failed, retrying with PowerShell...
    del "!_DL_ZIP!" >nul 2>&1
)
...
if errorlevel 1 (
    echo [haibot] ERROR: GitHub download also failed.
    echo [haibot] Download uv manually from: https://github.com/astral-sh/uv/releases/latest
    del "!_DL_ZIP!" >nul 2>&1
    exit /b 1
)
...
if defined ARG_UV_PATH (
    if not exist "%ARG_UV_PATH%" (
        echo [haibot] ERROR: Specified uv not found: %ARG_UV_PATH%
        exit /b 1
    )
    for %%I in ("%ARG_UV_PATH%") do set "PATH=%%~dpI;!PATH!"
    echo [haibot] uv found: %ARG_UV_PATH%
    goto :ensure_uv_done
)
...
for %%c in ("%USERPROFILE%\.local\bin\uv.exe" "%USERPROFILE%\.cargo\bin\uv.exe" "%LOCALAPPDATA%\uv\uv.exe") do (
    if exist %%c (
        ...
        echo [haibot] uv found: %%~c
        goto :ensure_uv_done
    )
)

REM 3. Try astral.sh (standard installer, fast outside China)
echo [haibot] Installing uv via astral.sh...
...
echo [haibot] astral.sh unreachable, falling back to GitHub Releases...
call :download_uv_github
if errorlevel 1 (
    echo [haibot] ERROR: Failed to install uv automatically.
    echo [haibot] Please install uv manually: https://docs.astral.sh/uv/
    exit /b 1
)
goto :ensure_uv_done
...
where uv >nul 2>&1
if errorlevel 1 (
    echo [haibot] ERROR: Failed to install uv. Please install it manually: https://docs.astral.sh/uv/
    exit /b 1
)
echo [haibot] uv installed via astral.sh
...
set "_CONSOLE_DEST=%_REPO_DIR%\src\haibot\console"
...
if exist "%_CONSOLE_SRC%\index.html" (
    echo [haibot] Copying console frontend assets...
...
if not exist "%_REPO_DIR%\console\package.json" (
    echo [haibot] WARNING: Console source not found - the web UI won't be available.
    exit /b 0
)

where npm >nul 2>&1
if errorlevel 1 (
    echo [haibot] WARNING: npm not found - skipping console frontend build.
    echo [haibot] WARNING: Install Node.js from https://nodejs.org/ then re-run this installer,
    echo [haibot] WARNING: or run 'cd console ^&^& npm ci ^&^& npm run build' manually.
    exit /b 0
)

echo [haibot] Building console frontend (npm ci ^&^& npm run build)...
pushd "%_REPO_DIR%\console"
npm ci
if errorlevel 1 (
    popd
    echo [haibot] WARNING: npm ci failed - the web UI won't be available.
    exit /b 0
)
npm run build
if errorlevel 1 (
    popd
    echo [haibot] WARNING: npm run build failed - the web UI won't be available.
    exit /b 0
)
popd
...
if exist "%_CONSOLE_SRC%\index.html" (
    ...
    set "CONSOLE_COPIED=1"
    set "CONSOLE_AVAILABLE=1"
    echo [haibot] Console frontend built successfully
    exit /b 0
)

echo [haibot] WARNING: Console build completed but index.html not found - the web UI won't be available.
exit /b 0
...
REM %~1 = RepoDir
if "%CONSOLE_COPIED%"=="1" (
    set "_CLEANUP_DEST=%~1\src\haibot\console"
...
REM ══════════════════════════════ MAIN ═════════════════════════════════════════
:main
echo [haibot] Installing HaiBot into %HAIBOT_HOME%
...
if exist "%HAIBOT_VENV%" (
    echo [haibot] Existing environment found, upgrading...
) else (
    echo [haibot] Creating Python %PYTHON_VERSION% environment...
)

uv venv "%HAIBOT_VENV%" --python %PYTHON_VERSION% --quiet --clear
if errorlevel 1 (
    echo [haibot] ERROR: Failed to create virtual environment
    exit /b 1
)

set "VENV_PYTHON=%HAIBOT_VENV%\Scripts\python.exe"
...
echo [haibot] Python environment ready (%PY_VERSION%)

REM ──── Step 3: Install HaiBot ──────────────────────────────────────────────────────────────────────────────────────────────────────────
set "EXTRAS_SUFFIX="
if defined ARG_EXTRAS set "EXTRAS_SUFFIX=[%ARG_EXTRAS%]"

set "VENV_HAIBOT=%HAIBOT_VENV%\Scripts\haibot.exe"
...
goto :install_from_github_haibot

:install_from_local
for %%I in ("%ARG_SOURCE_DIR%") do set "ARG_SOURCE_DIR=%%~fI"
echo [haibot] Installing HaiBot from local source: %ARG_SOURCE_DIR%
call :prepare_console "%ARG_SOURCE_DIR%"
echo [haibot] Installing package from source...
...
call :cleanup_console "%ARG_SOURCE_DIR%"
if %_INST_ERR% neq 0 (
    echo [haibot] ERROR: Installation from source failed
    exit /b 1
)
goto :install_verify

:install_from_github_haibot
where git >nul 2>&1
if errorlevel 1 (
    echo [haibot] ERROR: git is required for -FromSource without a local directory.
    echo [haibot]        Please install Git from https://git-scm.com/ or pass a local path:
    echo [haibot]        install-w-uv.bat -FromSource -SourceDir C:\path\to\HaiBot
    exit /b 1
)
echo [haibot] Installing HaiBot from source (GitHub)...
set "CLONE_DIR=%TEMP%\haibot-install-%RANDOM%"
git clone --depth 1 %HAIBOT_REPO% "%CLONE_DIR%"
if errorlevel 1 (
    if exist "%CLONE_DIR%" rd /s /q "%CLONE_DIR%"
    echo [haibot] ERROR: Failed to clone repository
    exit /b 1
)
...
if defined ARG_VERSION (
    ...
    set "_PACKAGE=haibot%ARG_VERSION%"
)
rem === End Version Validation ===

echo [haibot] Installing %_PACKAGE%%EXTRAS_SUFFIX% from PyPI...
...
uv pip install "%_PACKAGE%%EXTRAS_SUFFIX%" --python "%VENV_PYTHON%" --prerelease=allow --quiet --refresh-package haibot
if errorlevel 1 (
    echo [haibot] ERROR: Installation failed
    exit /b 1
)
...
if not exist "%VENV_HAIBOT%" (
    echo [haibot] ERROR: Installation failed: haibot CLI not found in venv
    exit /b 1
)
echo [haibot] HaiBot installed successfully

REM Check console availability (for PyPI installs, probe the installed package)
if "%CONSOLE_AVAILABLE%"=="0" (
    "%VENV_PYTHON%" -c "import importlib.resources, haibot; p=importlib.resources.files('haibot')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" > "%TEMP%\_haibot_console_check.tmp" 2>&1
    set /p CONSOLE_CHECK=<"%TEMP%\_haibot_console_check.tmp"
    del "%TEMP%\_haibot_console_check.tmp" >nul 2>&1
    if "!CONSOLE_CHECK!"=="yes" set "CONSOLE_AVAILABLE=1"
)

REM ──── Step 4: Create wrapper scripts ────────────────────────────────────────────────────────────────────────────────────────
if not exist "%HAIBOT_BIN%" mkdir "%HAIBOT_BIN%"

REM PowerShell wrapper
set "WRAPPER_PS1=%HAIBOT_BIN%\haibot.ps1"
echo # HaiBot CLI wrapper -- delegates to the uv-managed environment. > "%WRAPPER_PS1%"
echo $ErrorActionPreference = "Stop" >> "%WRAPPER_PS1%"
echo. >> "%WRAPPER_PS1%"
echo $CopawHome = if ($env:HAIBOT_HOME) { $env:HAIBOT_HOME } else { Join-Path $HOME ".haibot" } >> "%WRAPPER_PS1%"
echo $RealBin = Join-Path $CopawHome "venv\Scripts\haibot.exe" >> "%WRAPPER_PS1%"
echo. >> "%WRAPPER_PS1%"
echo if (-not (Test-Path $RealBin)) { >> "%WRAPPER_PS1%"
echo     Write-Error "HaiBot environment not found at $CopawHome\venv" >> "%WRAPPER_PS1%"
echo     Write-Error "Please reinstall: irm ^<install-url^> ^| iex" >> "%WRAPPER_PS1%"
echo     exit 1 >> "%WRAPPER_PS1%"
echo } >> "%WRAPPER_PS1%"
echo. >> "%WRAPPER_PS1%"
echo ^& $RealBin @args >> "%WRAPPER_PS1%"
echo [haibot] Wrapper created at %WRAPPER_PS1%

REM CMD wrapper
set "WRAPPER_CMD=%HAIBOT_BIN%\haibot.cmd"
echo @echo off > "%WRAPPER_CMD%"
echo REM HaiBot CLI wrapper -- delegates to the uv-managed environment. >> "%WRAPPER_CMD%"
echo set "HAIBOT_HOME=%%HAIBOT_HOME%%" >> "%WRAPPER_CMD%"
echo if "%%HAIBOT_HOME%%"=="" set "HAIBOT_HOME=%%USERPROFILE%%\.haibot" >> "%WRAPPER_CMD%"
echo set "REAL_BIN=%%HAIBOT_HOME%%\venv\Scripts\haibot.exe" >> "%WRAPPER_CMD%"
echo if not exist "%%REAL_BIN%%" ( >> "%WRAPPER_CMD%"
echo     echo Error: HaiBot environment not found at %%HAIBOT_HOME%%\venv ^>^&2 >> "%WRAPPER_CMD%"
echo     echo Please reinstall ^>^&2 >> "%WRAPPER_CMD%"
echo     exit /b 1 >> "%WRAPPER_CMD%"
echo ) >> "%WRAPPER_CMD%"
echo "%%REAL_BIN%%" %%* >> "%WRAPPER_CMD%"
echo [haibot] CMD wrapper created at %WRAPPER_CMD%
...
set "check_str=;%HAIBOT_BIN%;"
if /i "%path_check%" neq "%path_check:%check_str%=%" (
    echo [haibot] %HAIBOT_BIN% already in PATH
) else (
    :: === 修复1：安全传递参数（解决命令注入） ===
    if defined CURRENT_USER_PATH (
        powershell -NoProfile -Command "$p = $args[0]; $v = $args[1]; [Environment]::SetEnvironmentVariable('Path', $p + ';' + $v, 'User')" "%HAIBOT_BIN%" "!CURRENT_USER_PATH!"
    ) else (
        powershell -NoProfile -Command "$p = $args[0]; [Environment]::SetEnvironmentVariable('Path', $p, 'User')" "%HAIBOT_BIN%"
    )

    :: === 修复2：添加关键错误检查（解决失败不报错） ===
    if errorlevel 1 (
        echo [error] Failed to update PATH. HAIBOT_BIN: "%HAIBOT_BIN%"
        echo [error] Please verify the path is valid.
        exit /b 1
    )

    :: === 修复3：安全更新当前进程PATH ===
    set "PATH=%HAIBOT_BIN%;!PATH!"
    echo [haibot] Added %HAIBOT_BIN% to PATH
)

REM ──── Done ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
echo.
echo HaiBot installed successfully!
echo.
echo   Install location:  %HAIBOT_HOME%
...
echo.
echo   haibot init       # first-time setup
echo   haibot app        # start HaiBot
echo.
echo To upgrade later, re-run this installer.
echo To uninstall, run: haibot uninstall
