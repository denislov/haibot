...
def _migrate_workspace_items_from_source(
    ...
    Args:
        source_dir: Source directory (e.g., ~/.haibot or WORKING_DIR)
...
def ensure_qa_agent_exists() -> None:
    ...
    ``haibot_source_index``), and built-in tools are restricted (see
    ``build_qa_agent_tools_config``).
    ...
    agent_config = AgentProfileConfig(
        ...
        description=(
            "Builtin Q&A helper for HaiBot setup, local config under "
            "HAIBOT_WORKING_DIR, and documentation. Prefer reading files "
