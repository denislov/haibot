...
variable ``HAIBOT_AUTH_ENABLED`` is set to a truthy value (``true``,
...
def is_auth_enabled() -> bool:
    ...
    Returns ``True`` when ``HAIBOT_AUTH_ENABLED`` is set to a truthy
...
def auto_register_from_env() -> None:
    ...
    Called once during application startup.  If ``HAIBOT_AUTH_ENABLED``
    is truthy and both ``HAIBOT_AUTH_USERNAME`` and ``HAIBOT_AUTH_PASSWORD``
    ...
    username = os.environ.get("HAIBOT_AUTH_USERNAME", "").strip()
    password = os.environ.get("HAIBOT_AUTH_PASSWORD", "").strip()
