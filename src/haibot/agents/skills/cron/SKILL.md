...
# 查看任务状态
haibot cron state <job_id> --agent-id <agent_id>

# 创建任务
haibot cron create --agent-id <agent_id> ...

# 删除任务
haibot cron delete <job_id> --agent-id <agent_id>

# 暂停 / 恢复任务
haibot cron pause <job_id> --agent-id <agent_id>
haibot cron resume <job_id> --agent-id <agent_id>

# 立即执行一次已有任务
haibot cron run <job_id> --agent-id <agent_id>
```
...
```bash
haibot cron create --agent-id <agent_id> -f job_spec.json
```
...
- 缺少参数时，先问用户再创建
- 修改/暂停/删除前，先 `haibot cron list --agent-id <agent_id>`
- 排查问题时，用 `haibot cron state <job_id> --agent-id <agent_id>`
