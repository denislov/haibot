...
def _get_builtin_skill_names() -> list[str]:
    """Get list of builtin skill names from src/haibot/agents/skills/."""
...
def read_skill_requirements(skill_dir: Path) -> SkillRequirements:
    ...
    elif "haibot" in metadata:
        requires = metadata["haibot"].get("requires", {})
    else:
...
def _skill_config_env_var_name(skill_name: str) -> str:
    ...
    return f"HAIBOT_SKILL_CONFIG_{''.join(normalized).strip('_') or 'DEFAULT'}"
...
def apply_skill_config_env_overrides(
    ...
    ``HAIBOT_SKILL_CONFIG_<SKILL_NAME>`` (JSON string).
    """
...
def _extract_zip_skills(data: bytes) -> tuple[Path, list[tuple[Path, str]]]:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="haibot_skill_upload_"))
...
def _staged_skill_dir(skill_name: str) -> Iterator[Path]:
    ...
    temp_root = Path(
        tempfile.mkdtemp(prefix=f"haibot_skill_stage_{skill_name}_"),
    )
