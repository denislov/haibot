# -*- coding: utf-8 -*-
"""haibot uninstall — remove the HaiBot environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove HaiBot PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# HaiBot" comment line and the export PATH line
    cleaned = re.sub(
        r"\n?# HaiBot\nexport PATH=\"\$HOME/\.haibot/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove HaiBot environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL HaiBot data in {wd}")
    else:
        click.echo(
            "This will remove the HaiBot Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("HaiBot uninstalled. Please restart your terminal.")
