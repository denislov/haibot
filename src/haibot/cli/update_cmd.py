...
from .process_utils import (
    ...
    _is_haibot_service_command,
    _process_table,
)

_PYPI_JSON_URL = "https://pypi.org/pypi/haibot/json"
...
class InstallInfo:
    """Information about the current HaiBot installation."""
...
class RunningServiceInfo:
    """Detected HaiBot service endpoint state."""
...
def _fetch_latest_version() -> str:
    """Fetch the latest published HaiBot version from PyPI."""
    try:
        resp = httpx.get(
            _PYPI_JSON_URL,
            timeout=10.0,
        ...
        raise click.ClickException(
            f"Failed to fetch the latest HaiBot version from PyPI: {exc}",
        ) from exc
    except json.JSONDecodeError as exc:
        raise click.ClickException(
            "Received an invalid response from PyPI when checking for the "
            f"latest HaiBot version: {exc}",
        ) from exc
    version = str(data.get("info", {}).get("version", "")).strip()
    if not version:
        raise click.ClickException(
            "Unable to determine the latest HaiBot version.",
        )
    return version
...
def _detect_installation() -> InstallInfo:
    ...
    dist = metadata.distribution("haibot")
...
def _probe_service(base_url: str) -> RunningServiceInfo:
    """Probe a possible running HaiBot HTTP service."""
    try:
        resp = httpx.get(
...
def _process_candidate_ports() -> list[int]:
    """Infer candidate local HaiBot service ports from running processes."""
    ports: list[int] = []
    for _pid, command in _process_table():
        if not _is_haibot_service_command(command):
            continue
...
def _detect_running_service(
    ...
    """Detect whether a HaiBot HTTP service is currently running."""
    candidates: list[str] = []
    seen: set[str] = set()
    preferred_hosts: list[str] = []
...
def _running_service_display(running: RunningServiceInfo) -> str:
    ...
    if not running.base_url:
        return "a running HaiBot service"
    version_suffix = f" (version {running.version})" if running.version else ""
    return f"HaiBot service at {running.base_url}{version_suffix}"


def _confirm_force_shutdown(running: RunningServiceInfo) -> bool:
    """Ask whether `haibot shutdown` should be used before updating."""
    click.echo("")
    click.secho("!" * 72, fg="yellow", bold=True)
    click.secho(
        "WARNING: RUNNING HAIBOT SERVICE DETECTED",
        fg="yellow",
        bold=True,
    )
    click.secho("!" * 72, fg="yellow", bold=True)
    ...
    click.secho(
        "Running `haibot shutdown` will forcibly terminate the current "
        "HaiBot backend/frontend processes.",
        fg="red",
        bold=True,
    )
    click.secho(
    ...
    return click.confirm(
        "Run `haibot shutdown` now and continue with the update?",
        default=False,
    )
...
def _run_shutdown_for_update(
    ...
    """Run `haibot shutdown` in the current environment before updating."""
    command = [info.python_executable, "-m", "haibot"]
    ...
    click.echo("")
    click.echo("Running `haibot shutdown` before updating...")
    ...
    try:
        ...
        raise click.ClickException(
            "Failed to run `haibot shutdown`: " f"{exc}",
        ) from exc
    ...
    if result.returncode != 0:
        raise click.ClickException(
            "`haibot shutdown` failed. Please stop the running HaiBot "
            "service manually before running `haibot update`.",
        )
...
def _build_upgrade_command(
    ...
    package_spec = f"haibot=={latest_version}"
...
def _spawn_update_worker(
    ...
    *,
    ...
    worker_code = (
        "from haibot.cli.update_cmd import run_update_worker; "
        "import sys; "
...
def _run_update_worker_foreground(plan_path: Path) -> int:
    ...
    try:
        ...
        click.echo("")
        click.echo("[haibot] Update interrupted. Stopping installer...")
...
def run_update_worker(plan_path: str | Path) -> int:
    ...
    click.echo("")
    click.echo(
        "[haibot] Updating HaiBot "
        f"{plan['current_version']} -> {plan['latest_version']}...",
    )
    click.echo(f"[haibot] Using installer: {plan['installer_label']}")
    ...
    try:
        ...
        click.echo(f"[haibot] Update failed: {exc}")
        return_code = 1
    finally:
        try:
    ...
    if return_code == 0:
        click.echo("[haibot] Update completed successfully.")
        click.echo(
            "[haibot] Please restart any running HaiBot service "
            "to use the new version.",
        )
    else:
        click.echo(f"[haibot] Update failed with exit code {return_code}.")
        click.echo(
            "[haibot] Please fix the error above and run "
            "`haibot update` again.",
        )
...
def update_cmd(ctx: click.Context, yes: bool) -> None:
    """Upgrade HaiBot in the current Python environment."""
    ...
    if version_check is False:
        click.echo("HaiBot is already up to date.")
        return
    ...
    if running.is_running:
        if yes:
            raise click.ClickException(
                ...
                "Please stop it before running `haibot update`, or rerun "
                "without `--yes` to confirm a forced `haibot shutdown`.",
            )
        ...
        if running.is_running:
            raise click.ClickException(
                "Detected "
                f"{_running_service_display(running)} after `haibot shutdown`. "
                "Please stop it manually before running `haibot update`.",
            )

    if not yes and not click.confirm(
        f"Update HaiBot to {latest_version} in the current environment?",
        default=True,
    ):
        click.echo("Cancelled.")
        return
