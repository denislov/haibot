...
def _build_provider_info(
    ...
    # Providers with a default_base_url are "configured" when api_key is set.
    # Pure custom providers (no default_base_url) are "configured" when base_url is set.
...
async def set_active_model(
    ...
    # - Providers that have a default_base_url (built-in or Anthropic-like) need an api_key.
    # - Pure custom providers (no default_base_url) need base_url to be set.
    data = load_providers_json()
    settings = data.providers.get(body.provider_id)
    if provider.default_base_url:
        # Built-in provider (or one with a known default URL) — require api_key
        ...
        # Pure custom provider — require base_url
