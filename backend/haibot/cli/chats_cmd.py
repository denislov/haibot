...
def chats_group() -> None:
    ...
    \b
    常用示例：
      haibot chats list                          # 列出所有会话
      haibot chats list --user-id alice           # 按用户筛选
      haibot chats get <chat_id>       # 查看详情
      haibot chats create --session-id s1 --user-id u1
      haibot chats delete <chat_id>               # 删除指定会话
    """
...
def get_chat(
    ...
    \b
    CHAT_ID  会话 UUID，可通过 `haibot chats list` 获取。
...
def create_chat(
    ...
    内联创建示例：
      ...
      haibot chats create --session-id s1 --user-id u1 --channel imessage
