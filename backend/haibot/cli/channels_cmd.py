...
def _get_channel_names() -> dict[str, str]:
    """Return channel names filtered by HAIBOT_ENABLED_CHANNELS."""
...
def get_channel_configurators() -> dict:
    """Return channel configurators filtered by HAIBOT_ENABLED_CHANNELS."""
    available = get_available_channels()
    return {
...
def list_cmd() -> None:
    ...
    if not config_path.is_file():
        ...
        click.echo("Run `haibot channels config` to create one.")
        cfg = load_config()
    else:
